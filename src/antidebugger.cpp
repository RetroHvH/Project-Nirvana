...
{
	return;
	__oMsg("== TERMINATE ALL == ExitProcess : xxx");
...
void CAntiDebugger::ANTI_DEBUG_TOOL() // anti OllyDbg, MHS, Cheat Engine, TSearch, ArtMoney, IDA, WinDbg
{
	...
	bool hasOlly1 = CheckFindWindow((sz_fengyue), (sz__OllyDbg)); //OLLYDBG FENGYUE
	if (hasOlly1) TERMINATE_ALL();
	bool hasOlly2 = CheckFindWindow((sz_OLLYDBG), (sz_OllyDbg)); //OLLYDBG ORG
	if (hasOlly2) TERMINATE_ALL();
	bool hasOlly3 = CheckFindWindow((sz__DEROX), (sz__OllyDbg));
	if (hasOlly3) TERMINATE_ALL();
	bool hasOlly4 = CheckFindWindow((sz___CiS_), (sz__OllyDbg));
	if (hasOlly4) TERMINATE_ALL();
	bool hasOlly5 = CheckFindWindow((sz__MTCTOly), (sz__OllyDbg));
	if (hasOlly5) TERMINATE_ALL();
	bool hasMHS = CheckFindWindow((sz__Calc), (sz__MHS));
	if (hasMHS) TERMINATE_ALL();
	bool hasCE = CheckFindWindow((sz__Window), (sz__Cheat_Engine));
	if (hasCE) TERMINATE_ALL();
	bool hasTSearch = CheckFindWindow((sz__Afx_400000_8_10011_0_a0655), (sz__TSearch));
	if (hasTSearch) TERMINATE_ALL();
	bool hasArtMoney = CheckFindWindow((sz__TForm1), (sz__ArtMoney_Pro));
	if (hasArtMoney) TERMINATE_ALL();
	bool hasIDA = CheckFindWindow((sz__QWidget), (sz__IDA_Pro));
	if (hasIDA) TERMINATE_ALL();
	bool hasWinDbg = CheckFindWindow((sz__WinDbgFrameClass), (sz__WinDbg));
